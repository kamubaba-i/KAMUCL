...
const fixtureServer={id:'test-server',name:'测试服务器',address:'example.invalid:25565',versionId:versions[0].id,folder};let stallPreparation=false;
const calls = [], errors = []
...
ipcMain.handle('design:invoke', (_event, channel, ...args) => {
  ...
  switch (channel) {
    ...
    case 'servers:syncFromDat': return {list:[fixtureServer],targets:versions,added:0,updated:0};
    case 'servers:list': return [fixtureServer];
    case 'servers:ping': return {online:true,motd:'测试服务器',players:'1 / 20',latencyMs:20,version:'26.2'};
    case 'servers:prepareLaunch': return stallPreparation ? new Promise(r=>setTimeout(()=>r({versionId:versions[0].id,folder,address:fixtureServer.address,directJoin:true}),17000)) : {versionId:versions[0].id,folder,address:fixtureServer.address,directJoin:true};
    case 'game:launch': _event.sender.send('event:launchState',{status:'running',versionId:args[0],folder:args[2],launchId:'fixture-launch',text:'游戏进程已启动'});return null;
    case 'frp:status': return {accessKey:'fixture',tunnels:[{id:'sakura-123',name:'我的生存世界',nodeName:'测试节点',status:'running',desired:true,config:{tunnelId:123,localPort:25565},localIp:'127.0.0.1',remoteAddress:'example.invalid:25565',message:'隧道已连接',logs:[]}]};
    ...
    case 'skin:profile': return {"skins":[{"id":"test","variant":"classic","dataUrl":"data:image/png;base64,"}],"capes":[]}
...
app.whenReady().then(async()=>{
 ...
 await nav('friends');await run(`document.querySelectorAll('.pick-card')[1].click()`);await wait(500);
 assert.equal(await run(`document.querySelectorAll('.reference-links a').length`),3);await shot('voxlink-links');
 await click('← 更换方式');await run(`document.querySelectorAll('.pick-card')[0].click()`);await wait(500);
 for(const theme of ['blue-white','black-orange','pink-white']){
   settings.theme=theme;await win.reload();await wait(1300);await nav('friends');await run(`document.querySelectorAll('.pick-card')[0]?.click()`);await wait(300);await shot('frp-'+theme);
   const style=await run(`(()=>{const e=document.querySelector('.reference-links a');return {decoration:getComputedStyle(e).textDecorationLine,height:e.getBoundingClientRect().height}})()`);
   assert.equal(style.decoration,'none');assert(style.height>=36);
 }
 win.setSize(920,760);await shot('frp-narrow');win.setSize(1440,1000);
 await nav('servers');await wait(500);
 await run(`document.querySelector('.server-connect').click()`);await wait(700);
 assert(await run(`document.querySelector('.server-connect').textContent.includes('游戏已运行')`));
 assert.equal(requests.find(r=>r.channel==='game:launch').args[2],folder);await shot('server-running');
 win.webContents.send('event:launchState',{status:'exited',code:0,versionId:versions[0].id,folder,launchId:'fixture-launch'});await wait(300);
 stallPreparation=true;const before=requests.filter(r=>r.channel==='game:launch').length;
 await run(`document.querySelector('.server-connect').click()`);await wait(15500);
 assert(await run(`document.body.innerText.includes('服务器启动准备超时')`));
 await wait(2200);assert.equal(requests.filter(r=>r.channel==='game:launch').length,before);
 assert.deepEqual(errors,[]);fs.writeFileSync(path.join(root,'result.json'),JSON.stringify({pass:true,checks:['VoxLink links','themed FRP links','narrow layout','running label','locked folder','timeout with no late launch']},null,2));console.log('PASS 1067 production UI: '+root);win.destroy();app.quit()
