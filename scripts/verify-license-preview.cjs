...
app.whenReady().then(async()=>{
 ...
 await nav('skins'); await wait(500);
 assert(await run(`!!document.querySelector('.viewer3d canvas')`),'skin canvas missing');
 assert(!await run(`document.querySelector('.viewer3d-fallback')!==null`));
 await shot('replacement-skin-light');
 await click('待机');
 await run(`document.querySelector('.viewer3d').dispatchEvent(new WheelEvent('wheel',{deltaY:-150,bubbles:true,cancelable:true}))`);await wait(400);
 await run(`document.querySelector('.viewer3d').dispatchEvent(new MouseEvent('dblclick',{bubbles:true}))`);await wait(400);
 await run(`document.documentElement.setAttribute('data-theme','dark')`);await shot('replacement-skin-dark');
 await nav('settings');
 const text=await run(`document.body.innerText`);assert(text.includes('第三方'));
 assert.deepEqual(errors,[]);console.log('PASS replacement preview: '+root);win.destroy();app.quit()
