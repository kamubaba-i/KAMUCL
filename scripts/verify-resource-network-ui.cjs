...
app.whenReady().then(async()=>{
 ...
 await nav('mods');assert.equal(await run(`document.querySelectorAll('.fm-row').length`),100)
 assert(!calls.some(c=>/duplicates/i.test(c)),'Opening resource page must not scan archives')
 await shot('mods-3000');await click('下一页');assert(await run(`document.querySelector('.fm-row').textContent.includes('0100.jar')`))
 for(const key of ['packs','shaders']){await nav(key);assert.equal(await run(`document.querySelectorAll('.fm-row').length`),100)}
 for(const call of requests.filter(r=>r.channel==='fs:list')){assert.equal(call.args[1],folder);assert(call.args[0].startsWith('versions/'+versions[0].id+'/'))}
 await nav('keys');await run(`document.querySelector('.mouse-shortcut').click()`);await wait(200);await run(`const input=document.querySelector('[aria-label="鼠标灵敏度"]');input.value='125';input.dispatchEvent(new Event('change',{bubbles:true}))`);await wait(200);assert.equal(gameOptions.getDefaultGameOptions().values.mouseSensitivity,125);await shot('sensitivity')
 await nav('friends');await run(`document.querySelectorAll('.pick-card')[2].click()`);await wait(500)
 assert(!calls.includes('tc:install'));await shot('terracotta-manual');await click('下载陶瓦工具');assert(calls.includes('tc:install'))
 await click('← 更换方式');await run(`document.querySelectorAll('.pick-card')[1].click()`);await wait(500)
 assert(await run(`!![...document.querySelectorAll('.session-actions button')].find(b=>b.textContent.includes('TURN'))`));await shot('voxlink-20s')
 await click('退出房间');assert(calls.includes('voxlink:stop'))
 ...
 await run(`const i=document.querySelector('input[type=password]');i.value='fixture';i.dispatchEvent(new Event('input',{bubbles:true}))`);await wait(100)
 await click('读取我的隧道与节点');await shot('frp-guide')
 versions.length=0;await win.reload();await wait(1000);const before=calls.filter(c=>c==='fs:list').length
 for(const key of ['mods','packs','shaders']){await nav(key);assert(await run(`document.querySelector('.empty').textContent.includes('没有可选版本')`));assert.equal(calls.filter(c=>c==='fs:list').length,before)}
 await shot('empty-versions');assert.deepEqual(errors,[])
 fs.writeFileSync(path.join(root,'result.json'),JSON.stringify({pass:true,checks:['3000 entries paged','no hidden jar scan','scoped resource IPC','sensitivity save','manual Terracotta install','TURN after 20s','leave room','FRP guide','empty versions'],errors},null,2));console.log('PASS production resource/network UI: '+root);win.destroy();app.quit()
