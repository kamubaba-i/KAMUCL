...
app.whenReady().then(async()=>{
 ...
 await nav('mods');await wait(500);assert.equal(await run(`document.querySelectorAll('.fm-file-icon img').length`),2);assert(await run(`Array.from(document.querySelectorAll('.fm-file-icon img')).every(i=>i.complete && i.naturalWidth>0)`));await shot('mod-icons');
 const iconCalls=requests.filter(r=>r.channel==='mods:icons');assert.equal(iconCalls.at(-1).args[2],folder);assert.equal(iconCalls.at(-1).args[1].length,100);
 await click('下一页');await wait(500);assert(requests.filter(r=>r.channel==='mods:icons').at(-1).args[1][0]==='0100.jar');
 await nav('packs');const before=requests.filter(r=>r.channel==='mods:icons').length;await wait(500);assert.equal(requests.filter(r=>r.channel==='mods:icons').length,before);assert.equal(await run(`document.querySelectorAll('.fm-file-icon img').length`),0);
