...
const fixtureIcon = process.argv[2] ? 'data:image/png;base64,' + fs.readFileSync(process.argv[2]).toString('base64') : 'data:image/svg+xml;base64,'+Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><rect width="32" height="32" fill="green"/></svg>').toString('base64')
...
ipcMain.handle('design:invoke', (_event, channel, ...args) => {
  ...
  switch (channel) {
    case 'mods:checkUpdates': return {mcVersion:'26.2',loader:'fabric',entries:[{fileName:'0000.jar',name:'不同的内部名字',modId:'test',currentVersion:'1',sha1:'a',source:'modrinth',alreadyLatest:false,update:{projectId:'p',versionId:'v',versionNumber:'2',fileName:'new.jar',url:'https://example.invalid/new.jar'}}]};
    case 'mods:migrationPlan': return {id:'migration-fixture',sourceId:versions[0].id,folder,mcVersion:args[2],loader:args[3],loaderVersion:'1',createdAt:Date.now(),warnings:[],entries:[{fileName:'0000.jar',name:'内部名字',currentVersion:'1',sha1:'a',disabled:false,status:'compatible',target:{version:'2',fileName:'new.jar'}},{fileName:'0001.jar',name:'未匹配模组',currentVersion:'1',sha1:'b',disabled:true,status:'unavailable',reason:'该项目没有适配目标版本的文件'}]};
    case 'mods:migrationApply': return 'migration-task';
    case 'fs:toggleDisable': return [{name:'0000.jar.disabled',size:100,isDir:false,mtime:Date.now()}];
    case 'mods:icons': return Object.fromEntries(args[1].slice(0,2).map(name=>[name,fixtureIcon]));
    case 'fs:list': return Array.from({length:3000},(_,i)=>({name:String(i).padStart(4,'0')+(String(args[0]).includes('packs')?'.zip':'.jar'),size:100000,isDir:false,mtime:Date.now()}));
...
app.whenReady().then(async()=>{
 ...
 await nav('mods');await wait(500);assert.equal(await run(`document.querySelectorAll('input[role=switch]').length`),100);assert(await run(`document.querySelector('input[role=switch]').checked`));await click('检测更新');await wait(500);assert.equal(await run(`document.querySelector('.upd-name strong').textContent`),'0000.jar');assert.equal(await run(`document.querySelector('.upd-name small').textContent`),'不同的内部名字');assert(await run(`document.querySelector('.upd-row img').naturalWidth>0`));await shot('mod-switch-and-update');
 await click('版本迁移');await run(`(()=>{const e=document.querySelector('input[list="migration-versions"]');e.value='1.20.1';e.dispatchEvent(new Event('input',{bubbles:true}))})()`);await click('检查兼容性');assert(await run(`[...document.querySelectorAll('button')].find(b=>b.textContent==='确认迁移').disabled`));assert.equal(requests.filter(r=>r.channel==='mods:migrationApply').length,0);await shot('migration-confirmation');await run(`document.querySelector('.migration-warning input').click()`);await click('确认迁移');assert.deepEqual(requests.find(r=>r.channel==='mods:migrationApply').args,['migration-fixture',true]);
 await run(`document.querySelector('input[role=switch]').click()`);await wait(300);assert(requests.some(r=>r.channel==='fs:toggleDisable'));assert.equal(await run(`document.querySelector('input[role=switch]').checked`),false);
 assert.deepEqual(errors,[]);console.log('PASS paged mod icons UI: '+root);win.destroy();app.quit()
