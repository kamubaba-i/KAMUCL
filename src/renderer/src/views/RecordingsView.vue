...
watch(activeFolder, () => {
  generation++; selected.value = []; target.value = ''; results.value = []; confirm.value = ''; page.value = 1
  data.value = { entries: [], warnings: [], library: '', instances: [] }
  void refresh()
})
...
async function refresh(silent = false) {
  ...
  try { const next = await invoke<RecordingCatalog>('recordings:list'); if (disposed || id !== generation) return; data.value = next; selected.value = selected.value.filter(id => next.entries.some(e => e.id === id)); page.value = Math.min(page.value, pages.value); if (!silent) target.value = '' }
...
<template>
  <section class="recordings-page" data-ui="recordings:page">
    ...
    <div class="card recording-library" data-ui="recordings:library">
      <div><h3>集中收藏文件夹</h3><p class="muted path">{{ data.library || '正在读取…' }}</p><p class="muted">跟随当前游戏文件夹，自动汇总其中所有已安装实例及共享目录的录像；原文件保持原位。自定义位置或旧收藏的录像可手动导入。</p><p class="muted">按住录像名称即可拖到桌面或文件夹；勾选多项后可一起拖出。</p></div>
