...
> 更新时间：2026-09-10 03:38 ｜ 当前开发版本：**1.0.28**（本地修复，尚未提交、打包或发布）
> 工作区：E:\KAMUCL（git 仓库，含本批未提交修改）
...
**长期交付约定（用户最新授权）**：每批修改后默认完成版本递增、验证、打包，并将源码及对应 GitHub Release 更新到 `kamubaba-i/KAMUCL`；不再等待额外的打包/发布指令。项目根目录 `AGENTS.md` 保存此规则。当前正在补齐 1.0.28 的打包与发布。
...
用户确认版本规则：每批修复递增版本尾号，本批为 **1.0.27**；package.json、package-lock.json 根包版本与内置更新日志已同步。当前尚未提交、打包或发布。
...
**当前停点**：1.0.27、1.0.28 修改均保留在工作区，当前版本元数据为 1.0.28；后续提交、打包、发布尚未执行。
